...
for i in $KUBELET_ARGS; do
    ...
    case $i in
        ...
        ;;
        "--healthz-port=0")
            echo "Healthz already disabled"
            HEALTHZ_DISABLED=true
        ;;
    esac
done

if [ "$SERVER_DISABLED" == false ]; then
    echo "Disabling server"
    KUBELET_ARGS="$KUBELET_ARGS --enable-server=false"
fi
if [ "$HEALTHZ_DISABLED" == false ]; then
    echo "Disabling healthz"
    KUBELET_ARGS="$KUBELET_ARGS --healthz-port=0"
fi

if [ "$SERVER_DISABLED" == false ] || [ "$HEALTHZ_DISABLED" == false ]; then
